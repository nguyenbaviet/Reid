...
def load_pretrained_weights(model, weight_path,optimizer=None, scheduler=None, transfer_learning=False):
    ...
    if 'epoch' in checkpoint and not transfer_learning:
        epoch = checkpoint['epoch']
    else:
        epoch = 0
    if optimizer is not None and 'optimizer' in checkpoint.keys() and not transfer_learning:
        optimizer.load_state_dict(checkpoint['optimizer'])
        print('Loaded optimizer')
    if scheduler is not None and 'scheduler' in checkpoint.keys() and not transfer_learning:
        scheduler.load_state_dict(checkpoint['scheduler'])
        print('Loaded scheduler')
