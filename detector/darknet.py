...
def bbox2points(bbox):
    ...
    """
    x, y, w, h = bbox
    xmin = max(0, int(round(x - (w / 2))))
    xmax = int(round(x + (w / 2)))
    ymin = max(0, int(round(y - (h / 2))))
...
def draw_boxes(detections, image, colors, ids):
    import cv2
    for label, confidence, bbox in detections:
        ...
        cv2.putText(image, "{} [{:.2f}]".format(label, float(confidence)),
                    ...
                    colors[label], 2)
    for id in ids:
        if not id.is_confirmed() or id.time_since_update > 3:
            continue
        xmin, ymin, xmax, ymax = id.to_tlbr().astype(int)
        image = cv2.rectangle(image, (xmin, ymin), (xmax, ymax), (255, 0, 0), 2)
        image = cv2.putText(image, str(id.track_id), (xmin, ymin), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
    return image
def _draw_boxes(detections, image, colors):
    import cv2
    for label, confidence, bbox in detections:
        left, top, right, bottom = bbox2points(bbox)
        cv2.rectangle(image, (left, top), (right, bottom), colors[label], 1)
